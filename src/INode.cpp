...
INode::~INode()
{
    ...
    _outputSequence = std::function<void(std::string, uint32_t, PVariable)>();
...
void INode::outputSequence(uint32_t index, PVariable message)
{
    if(_outputSequence) _outputSequence(_id, index, message);
}
